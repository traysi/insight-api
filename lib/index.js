...
/**
 * A service for Ritocore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinShort = options.coinShort || 'RITO';
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/ritocoin/?convert=USD';
...
InsightAPI.dependencies = ['ritod', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.ritod.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.ritod.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'rito');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'rito');
        },
        extraEvents: ['rito/tx', 'rito/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.rito.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify rito subscribers
              for (var i = 0; i < self.subscriptions.rito.length; i++) {
                  self.subscriptions.rito[i].emit('rito/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.rito.length) {
       ...
       if (this.subscriptions.rito.length) {
           var transformedTrx = this.txController.transformRitoTransaction(tx);

           for (var i = 0; i < this.subscriptions.rito.length; i++) {
               this.subscriptions.rito[i].emit('rito/tx', transformedTrx);
           }
