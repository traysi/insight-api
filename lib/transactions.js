...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.ritod.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = ritocore.Address(output.address); //TODO return type from ritocore-node
...
TxController.prototype.transformRitoTransaction = function(transaction) {
